...
if platform.system() == 'Windows':#判断当前系统是不是Windows
    ...
    for stp in wmi.WMI().Win32_SystemEnclosure():#判断计算机类型
        if 'Virtual' in model or 'VMware' in model or 'QEMU' in model:
            systyp = '虚拟机'
    ...
    for sys in wmi.WMI().Win32_OperatingSystem():
        ...
        if sys.CSDVersion == None:
            if int(build) >= 10240:#判断是不是Windows10/11
                if int(build) == 10240:
                    sp = '1507'
                    ...
                    sp = '21H1'
                elif int(build) == 19044 or int(build) == 22000:
                    sp = '21H2'
                else:
    ...
    if os.path.exists("/sys/firmware/efi/efivars"):
        bootst = 'UEFI'
        firmtype = 'UEFI'
    elif os.path.exists("/sys/firmware/coreboot"):#判断是不是coreboot，但我手上没有coreboot的设备，不知道怎么判断，这里随便写了
        bootst = 'CoreBoot'
        firmtype = 'CoreBoot'
    elif 'x86' in bit:
        bootst = '传统'
